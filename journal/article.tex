...
described hindering the progress of the transformation.
Some of the challenges presented here were not explicitly presented as
transformation challenges, but rather as general difficulties in agile
imlementation. These issues are still included as creating an agile
implementation can be seen as a part of the transformation process.
Some solutions for challenges are also presented, but they should be considered
as ways of describing the challenges rather than direct success factors.
...
% Change resistance
% Lack of investment in transformation
% Agile is found to be hard
% Large organization issues
% Coordinating work between many teams
% Integrating other functions than development to agile
% Integrating requirements management and quality assurance
% Other challenges in transformation (zealots, falling back)


\subsubsection{Change resistance}

To start a transformation the organization needs an initial push. However,
people are usually disinclined to change, and it any organizational change
should anticipate some resistance. People will not be willing to change unless
...
be willing to change, and some employees will never adapt to the change [P6].
During a transformation period objections to change may become a major reason
for loss in time and productivity [P29].
...
and cautious which slows down any change. The organization was also described
having a deeply rooted status-quo because of high employee retention, which
further hindered change [P14]. In some cases it was reported that people worried
about new roles and responsibilities that agile would possibly bring [P44]. For
...
will show. In many cases the initiative to change came from management, but when
...
instance, Atlas [P3] describes how it would have required executive support to
integrate the product management office and separate quality assurance groups in
the agile process. In this case, when managers owning projects with many teams
were not involved in the transformation, the agile model was restrained from
spreading beyond team level [P3]. Lack of middle management support was also
...
organization. If there is no intention to allow resources for the transformation
problems will ensue. Typical examples on this was lack of investment in
training, and that people were kept allocated to tasks from the original
arrangement. In some cases the transformation was impeded when work spaces could
not be updated.
...
Agile was also thought being a solution to all problems [P9], success was
declared prematurely [P49], and expectations created by successful early
experiments were not fullfilled [P38]. Disapointment was granted when these
hopes were not fullfilled, but instead implementing the agile model was found
to be hard and time consuming. The difficulty in implementing agile is evident
in misconceptions of the agile model, when by-the-book implementations prove to
be insufficient, and when the agile methods were applied badly to the
organization.
...
scale development, and some practices required customization [P49]. The model of
agile should not be prescribed as a check list, as it would impose a single
...
vocabulary in favor of familiar terminology from the old model [P44]. A benefit
of introducing new vocabulary is that it may stimulate new ways of thinking
[P44].
...
creating a model to coordinate work between many teams.
% --> Present the different aspects of this topic?
...
some teams chose to focus only on their own goal [P7]. Independent teams that
did not respect the larger context they were working in obstructed coordination
[P13]. Coordination was also difficult when teams were working independently for
different customers but the applications being built were interdependent [P29].
One case reported that allowing teams to have different sprint durations created
delays in delivery [P51].
...
The primary studies revealed challenges which relate particularly to large
...
management. Also, as large organizations accomodate many development teams there
may be problems if the development models between teams are not synchronized.
Internal fragmenttation is a further problem that may be encountered in large
organizations.
...
The role of middle management was seen as unclear in agile methods [P2]. This is
problematic as an agile transformation requires cultural change especially on
the middle management level [P2].
...
side was seen as problematic [P1, P26], and causing tensions on all
organizational levels [P10]. Ongoing projects had been set up with previous
development methods, and the agile methods needed to be arranged to fit with
them [P8, P29]. A particular problem in collaborating between projects using
different models was that in Scrum the software design was fleshed out over time
as sprints progressed, but the waterfall model would require detailed design to
be locked early in the project [P29].
...
In addition to teams using agile and waterfall methods side by side it is
possible in a large organization that the agile interpretations of teams start
diverging. When many teams implement agile without consistent guidance friction
and fragmentation may be generated [P9]. The organization may require moving
people between teams from time to time, and therefore it would be desirable that
no big differences in teams' agile cultures exist. Divergence in process would
create risk increasing costs when relocating people [P38].
...
of these activities would be performed within the development team. The effort
required for managing requirements and QA for large systems may be too large for
individual teams to handle.
...
give only short term visibility, so organization specific practices for business
to do long term planning had to be created [P26, P31]. Several cases described
that care had to be taken to avoid long term planning becoming a prescriptive
...
In an enterprise setting quality assurance tasks will require effort beyond
