...
development methods than agile (e.g. [Sagesser 2013]), and general talk on
organizational transformation.

The facet of \emph{organizational transformation} was interpreted so that the
primary study must present insights on the process of transformation. Some
topics that could clearly be excluded were discussing a large agile effort but

-- not presenting the how the organization had evolved [Fitzgerald?],
   Lyon [Lyon 2008] was excluded as it discusses technical challenges in scaling
   up an agile process instead of providing organizational viewpoints.

-- pilot cases that clearly span one limited project [Scott 2008?, Mc Cormic 2005],

-- a single team in a large organization [Fulgham, Jensen].

-- Some case studies that provide minimal information on the transformation process [Greening?].
   [Mishra 2011] discusses agile practices in a large organization, but does not
   provide viewpoints on the transformation process and introduction of agile methods.
   Kim [Kim 2012] sounds promising at a start, but merely presents agile tools
   lacking any transformation description.
   Also [Hodgkins].
...
seen as a ``scaling up'' journey. However, we did not include discussions purely
on scaling up, especially if no organizational change was considered. [Maranzato
2012, Lyon 2008]
...
--> The case of difficult decision regarding size [Cloke 2007, Miller 2012].
--> [Tudor 2006] Exclude, borderline. There is no evidence on size. Many teams
    are mentioned, but there is still no direct indication of large size.

--> ``The team'' referring to a large organization [Hodgkins]
...
reach a decision. As a result of the full text filtering 45 papers were selected
to be included.
In addition 2 papers were included from the references of the evaluated papers,
and 3 papers were included from the results of the preliminary search. Finally,
50 papers were selected as primary studies.
...
\bibliography{sources_background,sources_primary}
