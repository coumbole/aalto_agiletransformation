...
studies \cite{Cruzes2011a}. Initially a list of codes was established for the
contextual information, as presented in Table \ref{table:contextualcodes}. The
codes related to the research questions were chosen to be created by an
inductive procedure. The reason for inducting codes rather than using an a
priori approach was to avoid the researchers' previous assumptions of the
research area to affect the choice of codes. To give direction for choosing the
codes seven different code families were established (see Table
\ref{table:codefamilies}), including a description defining the scope of the
families and examples for codes. The example codes would not necessarily be used
as actual codes, and would not represent the entire or final scope of the
families.
...
Agile practice & Agile practice or methodology used (eg. Scrum, XP) \\
...
Geographical\newline location & Where the organization is located. Includes
headquarters and multisite locations. \\

Large scale definition & A definition of "large scale" software development. \\

Measuring results of\newline transformation & Mentioning of how
transormation progress is measured, or how previous indicators have changed. \\

Satisfaction in agile & Internal or external satisfaction/trust/credibility is
mentioned. \\

Multisite / GSD & Mentioning of a multisite organization. This does not imply
that the multisite environment has brought special viewpoints on the agile
implementation. \\
...
\begin{table}[h]
    \begin{tabular}{ p{0.32\textwidth} p{0.65\textwidth} }
		...
		(coaching, piloting, continuous integration, communities of practice). Other
		distinguishable factors that are presented as neutral in the primary studies \\
		
		Other &
		Contextual codes, other codes not relating to RQ's. \\
...
coding process are presented in Table \ref{table:codecount}.

\begin{table}[h]
    \begin{tabular}{ l r r }
        \toprule
        Code family    &  Codes  &  Quotes
        \\
        \midrule
...
\footnotetext[2]{Due to technical reasons a small number of ``free quotations''
is included.}
